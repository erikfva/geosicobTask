...
app.listen(config.express.port, function () {
  console.log(
    "Servicios de geoprocesamiento disponibles en el puerto " +
      config.express.port +
      "!"
  );
...
app.get("/do/:id", (req, res, next) => {
  ...
  var idgeoproceso = isNaN(parseInt(req.params.id))
    ? -1
  ...
  pool.connect(function (err, client, done) {
    ...
    client.query(
      "SELECT idusuario, proceso, entrada, salida FROM registro_derecho.geoprocesamiento WHERE idgeoproceso =" +
        parseInt(idgeoproceso),
      function (err, result) {
        //call `done(err)` to release the client back to the pool (or destroy it if there is an error)
        done(err);
        ...
        if (err) {
          ...
          return res
            .status(500)
            .json({ success: 0, msg: "Error: " + err.message });
        }
        if (!result.rows.length) {
          return res.status(500).json({
            success: 0,
            msg: "Error: No se ha encontrado el proceso " + idgeoproceso,
          });
        }
        ...
        switch (result.rows[0].proceso) {
          case "sicob_build_shapefiles":
            ...
            sicob_log_geoproceso(
              '{"idgeoproceso":"' + idgeoproceso + '", "exec_point":"inicio"}',
              function (err, res) {
                if (err) {
                  console.log(
                    "sicob_build_shapefiles, idgeoproceso=" +
                      idgeoproceso +
                      ": Error al registrar el log inicial!!! " +
                      err
                  );
                }
              }
            ...
            sicob_build_shapefiles(result.rows[0].entrada, function (
              err,
              resJSON
            ) {
              if (err) {
                console.log(
                  "sicob_build_shapefiles(" +
                    idgeoproceso +
                    "): Al final ocurrio este error: " +
                    err
                );
              }
              //Registrando fin del proceso en el log.
              sicob_log_geoproceso(
                '{"success":"1","idgeoproceso":"' +
                  idgeoproceso +
                  '", "exec_point":"fin","payload":' +
                ...
                function (err, res) {
                  if (err) {
                    console.log(
                      "sicob_build_shapefiles(" +
                        idgeoproceso +
                        "): Error al registrar el log final!!! " +
                        err
                    );
                  }
                }
          ...
          case "sicob_analisis_sobreposicion":
            ...
            sicob_analisis_sobreposicion(result.rows[0].entrada, function (
              err,
              resJSON
            ) {
              if (err) {
              ...
              //console.log(resJSON)
              console.log(
                "sicob_analisis_sobreposicion(" + idgeoproceso + "): Termino."
              );
            });
            break;
          default:
            sicob_rungeoprocess(idgeoproceso, function (err, resJSON) {
              ...
              console.log(
                "sicob_rungeoprocess(" + idgeoproceso + "): Termino."
              );
            });
        }
        return res.status(200).json({
          success: true,
          msg: "Ejecutando geoproceso...",
          idgeoproceso: idgeoproceso,
        });
      }
    );
  });
...
app.post("/analisis", (req, res) => {
  var sql =
    "SELECT sicob_analisis_sobreposicion('" + JSON.stringify(req.body) + "')";
...
function batchSQL(sqlList, cb, finish) {
  ...
  var execNext = function () {
    ...
    console.log("ejecutando..." + sql)(sql, function (err, rows) {
      sqlPending--;
...
function sicob_rungeoprocess(idgeoproceso, cb) {
  ...
  var sql =
    "SELECT sicob_ejecutar_geoproceso('" + strparam + "'::json) AS _out";
  //var sql = 'SELECT sicob_ejecutar_geoproceso(\'{""idgeoproceso""":""' + parseInt(idgeoproceso) + '""}\'::json) AS _out';
  var cmd =
    "psql -d geodatabase -U " + config.postgres.user + ' -c "' + sql + '"';
...
function sicob_build_shapefiles(data, cb) {
  ...
  timesid +=
    (now.getMilliseconds() < 10
      ...
      ? "0"
      : "") + now.getMilliseconds().toString(); // pad with a 0
  ...
  const compressSHP = function () {
    zip({
      ...
      cb: function (err) {
        //(opcional) Funci�n de callback que se ejecuta al finalizar el trabajo.
        var result = {
          link: config.URL_SHP + fzipname + ".zip",
          shp_cnt: files_cnt,
          success: 1,
        };
        ...
        if (err) {
          error.message =
            error.message == ""
              ? "Error (sicob_build_shapefiles/zip):" + err
              : error.message + ", " + err;
  ...
  for (var i = 0; i < data.lyr_list.length; i++) {
    (function (i) {
      ...
      let condition =
        Object.keys(_condition).length === 0 ? "TRUE" : buildWHERE(_condition);
      ...
      let ogr2ogr = config.PATH_OGR2OGR.replace(/"/gi, "") + "ogr2ogr";
      var child = spawn(ogr2ogr, cmdParam);
...
function sicob_analisis_sobreposicion(op, cb) {
  ...
  if (doanalisys.length == 0) {
    //Si no se especifica el tipo de analisis entonces es un full analisis.
    op.doanalisys = ["ATE", "ASL", "PGMF", "POAF"];
    listSQL.push(
      "SELECT sicob_analisis_sobreposicion('" + JSON.stringify(op) + "')"
    );
...
function zip(op) {
  ...
  archive.glob(op.globpatt, {
    nodir: true,
    nomount: true,
    cwd: op.workdir || process.cwd(),
  });
...
function sicob_obtener_predio(op, cb) {
  ...
  var tbl_name =
    op.lyr_in.indexOf(".") !== -1 ? op.lyr_in.split(".")[1] : op.lyr_in;
  ...
  const do_configurar_capa_parcelas = new Promise(function (resolve, reject) {
    ...
    executeScalar(sql, function (err, res) {
      ...
      _fldpropietario = res.exist_propietario
        ? _fldpropietario
  ...
  const do_add_titulacion = function (lyr, condition) {
    return new Promise(function (resolve, reject) {
      ...
      var params = {
        ...
        b: "coberturas.parcelas_tituladas",
        subfix: "_tit",
        tolerance: "0",
        add_diff: false,
      };
  ...
  const do_add_pop = function (lyr, condition) {
    return new Promise(function (resolve, reject) {
      ...
      var params = {
        ...
        subfix: "_pop" + Math.floor(Math.random() * 100 + 1),
        tolerance: "0",
        add_diff: false,
      };
  ...
  const do_registrar_resultado = function (
    lyr_predio_conf,
    lyr_overlaying,
    condition
  ) {
    return new Promise(function (resolve, reject) {
      ...
      var _fldpredio =
        (lyr_predio_conf.lyr_parc
          ? lyr_predio_conf.lyr_parc.fldpredio
          : null) || "predio";
      var _fldpropietario =
        (lyr_predio_conf.lyr_parc
          ? lyr_predio_conf.lyr_parc.fldpropietario
          : null) || "propietario";
      ...
      var _fldsource_predio =
        (lyr_predio_conf.lyr_pred ? lyr_predio_conf.lyr_pred.source : null) ||
        (lyr_predio_conf.lyr_parc ? lyr_predio_conf.lyr_parc.source : null) ||
        "";
      var _fldid_predio =
        (lyr_predio_conf.lyr_parc
          ? lyr_predio_conf.lyr_parc.fldidpredio
          : null) || "id_b";
      ...
      switch (subfix) {
        ...
        case "_tioc":
          _fldtipo_propiedad =
            "CAST('Territorio Indígena Originario Campesino' as text)";
          break;
        case "_pop":
          _fldsup_predio =
            "CASE sup_pre > 0 WHEN TRUE THEN sup_pre ELSE sicob_sup END";
      ...
      executeSQL(sql, function (err, res) {
        if (err) {
          console.log(
            "do_registrar_resultado: Error ejecutando -> " + sql,
            err
          );
          throw err;
        }
        ...
        if (subfix == "_parc") {
          //Adicionar informacion de titulacion y de pop
          do_add_titulacion(
            lyr_parcelas,
            "source_predio = ''" + op.lyr_parc + "''"
          );
          do_add_pop(lyr_parcelas, "source_predio = ''" + op.lyr_parc + "''");
        }

        if (subfix == "_tit")
          //adicionando informacion de pop
          do_add_pop(
            lyr_parcelas,
            "source_parcela = ''coberturas.parcelas_tituladas''"
          );
        ...
        for (var i = 0, len = arr.length; i < len; i++) {
          row_inserted +=
            arr[i].command == "INSERT" || arr[i].command == "SELECT"
              ? arr[i].rowCount
              : 0;
        }
    ...
    return new Promise(function (resolve, reject) {
      ...
      const search_lyr_predio = function (lyrs) {
        if (!lyrs.length) {
          return reject(
            new Error(
              'search_lyrs_predio: No existen capas de predios configuradas en la variable "lyrs".'
            )
          );
        }
        ...
        //console.log(
        if (a !== "" && lyr_predio.lyr_parc) {
          //Si existen poligonos para localizar.
          var params = {
            a: a,
          ...
          executeScalar(sql, function (err, res) {
            ...
            if (parseInt(res.features_inters_cnt) > 0) {
              //--> Si se han localizado predios.
  ...
  do_configurar_capa_parcelas
    .then(function () {
      do_search_predios(lyrs_predio).then(function (res) {
        console.log("TERMINO!!!", res);

        if (res.features_diff_cnt > 0) {
          //finalizo con parcelas sin identificar predios
          do_actualizar_referencia(op.lyr_in, res.params.a, res) //--> Se actualiza la referencia (si es necesario).
            .then(function () {
              do_registrar_resultado(
                res.lyr_predio,
                res.lyr_over,
                "id_b IS NULL"
              ) //--> Se agregan los resultados a la capa resultado.
                .then(function (row_cnt) {
